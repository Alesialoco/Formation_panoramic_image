...
@dataclass
class StitchingParameters5:
    """Класс для хранения всех параметров сшивки для 5 камер"""
    ...
    blend_zone_45_start: int
    blend_zone_45_end: int
    ...
    @classmethod
    def load(cls, filepath: str) -> 'StitchingParameters5':
...
class OptimizedCylindricalStitcher5:
    """
    Класс для сшивки пяти видео в панораму
    """
    
    def __init__(self, video1_path: str = None, video2_path: str = None, 
                 video3_path: str = None, video4_path: str = None, video5_path: str = None,
                 output_path: str = None, num_calibration_frames: int = 10, 
                 neutral_plane_t: float = 0.5, fov_horizontal: float = 150, 
        ...
        self.blend_zone_45_start = self.blend_zone_45_end = 0
    ...
    def calculate_homographies_from_frames(self) -> None:
        ...
        cap5 = cv2.VideoCapture(self.video5_path)
        ...
        for i in range(self.num_calibration_frames):
            ...
            if not ret1 or not ret2 or not ret3 or not ret4 or not ret5:
                break
            ...
            if h > 360:
                ...
                frame5_small = cv2.resize(frame5, (new_w, 360))
            else:
                frame1_small = frame1
                frame2_small = frame2
                frame3_small = frame3
            ...
            H_4_to_5 = self.find_homography_orb(frame4_small, frame5_small)
            H_5_to_4 = self.find_homography_orb(frame5_small, frame4_small)
            ...
            if H_4_to_5 is not None:
                homographies_4_to_5.append(H_4_to_5)
            if H_5_to_4 is not None:
                homographies_5_to_4.append(H_5_to_4)
        ...
        if homographies_4_to_5:
            self.homography_4_to_5 = np.median(homographies_4_to_5, axis=0)
            self.homography_4_to_5 = self.homography_4_to_5 / self.homography_4_to_5[2, 2]
            logger.info("Гомография видео4->видео5 вычислена")
        else:
            raise Exception("Не удалось вычислить гомографию видео4->видео5")

        if homographies_5_to_4:
            self.homography_5_to_4 = np.median(homographies_5_to_4, axis=0)
            self.homography_5_to_4 = self.homography_5_to_4 / self.homography_5_to_4[2, 2]

    def compute_neutral_plane_transforms(self) -> None:
        """Вычисление преобразований к нейтральной плоскости"""
        logger.info(f"Создание преобразований к нейтральной плоскости (t={self.neutral_plane_t})...")

        H_3_to_4 = self.homography_3_to_4 / self.homography_3_to_4[2, 2]
        H_4_to_3 = self.homography_4_to_3 / self.homography_4_to_3[2, 2]
    ...
    def compute_all_transforms_to_neutral(self, frame1: np.ndarray, frame2: np.ndarray,
                                           frame3: np.ndarray, frame4: np.ndarray, 
                                           frame5: np.ndarray) -> None:
        ...
        self.transform_1_to_neutral = self.neutral_transform_3_to_neutral @ self.homography_2_to_3 @ self.homography_1_to_2
        self.transform_2_to_neutral = self.neutral_transform_3_to_neutral @ self.homography_2_to_3
        self.transform_3_to_neutral = self.neutral_transform_3_to_neutral
        self.transform_4_to_neutral = self.neutral_transform_4_to_neutral
        self.transform_5_to_neutral = self.neutral_transform_4_to_neutral @ self.homography_5_to_4
    ...
    def calculate_final_transforms(self, frame1: np.ndarray, frame2: np.ndarray,
        ...
        corners5 = np.array([[0, 0], [w5, 0], [w5, h5], [0, h5]], dtype=np.float32)
        corners5_transformed = cv2.perspectiveTransform(corners5.reshape(-1, 1, 2),
                                                        self.transform_5_to_neutral)

        all_corners = np.vstack([corners1_transformed, corners2_transformed,
                                  corners3_transformed, corners4_transformed, corners5_transformed])
        ...
        self.final_transform_5 = translation @ self.transform_5_to_neutral
    ...
    def precompute_blend_masks(self, frame1: np.ndarray, frame2: np.ndarray,
        ...
        scale = min(640 / max(frame3.shape[0], frame3.shape[1]), 1.0)
        if scale < 1.0:
            ...
            small5 = cv2.resize(frame5, (int(frame5.shape[1] * scale), int(frame5.shape[0] * scale)))
        else:
            small1, small2, small3, small4, small5 = frame1, frame2, frame3, frame4, frame5
        ...
        final_transform_5_small = scale_matrix @ self.final_transform_5 @ inv_scale_matrix
        ...
        warped4 = cv2.warpPerspective(small4, final_transform_4_small, (small_w, small_h),
                                       flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        warped5 = cv2.warpPerspective(small5, final_transform_5_small, (small_w, small_h),
                                       flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        ...
        self.blend_mask_45 = np.zeros((h, w), dtype=np.float32)
        ...
        if np.any(overlap_34):
            ...
            if len(overlap_cols) > 0:
                ...
                self._create_blend_mask(self.blend_mask_34, start, end, w, increasing=True)

        if np.any(overlap_45):
            overlap_cols = np.where(np.any(overlap_45, axis=0))[0]
            if len(overlap_cols) > 0:
                start = int(overlap_cols[0] / scale)
                end = int(overlap_cols[-1] / scale)
                self.blend_zone_45_start = start
                self.blend_zone_45_end = end
                self._create_blend_mask(self.blend_mask_45, start, end, w, increasing=False)
    ...
    def stitch_frame_optimized(self, frame1: np.ndarray, frame2: np.ndarray,
        ...
        warped4 = cv2.warpPerspective(frame4, self.final_transform_4, self.output_size,
                                       flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        warped5 = cv2.warpPerspective(frame5, self.final_transform_5, self.output_size,
                                       flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        ...
        only4 = mask4 & ~mask3 & ~mask5
        result[only4] = warped4[only4]

        # Стык 4-5
        overlap_45 = mask4 & mask5
        if np.any(overlap_45):
            blend_mask_3d = np.stack([self.blend_mask_45] * 3, axis=2)
            result[overlap_45] = (warped4[overlap_45] * (1 - blend_mask_3d[overlap_45]) + 
                                   warped5[overlap_45] * blend_mask_3d[overlap_45]).astype(np.uint8)
        
        only5 = mask5 & ~mask4
        result[only5] = warped5[only5]
    ...
    def initialize_stitching_parameters(self) -> None:
        ...
        logger.info("Инициализация параметров сшивки для 5 видео...")
        ...
        self.compute_all_transforms_to_neutral(frame1, frame2, frame3, frame4, frame5)
        self.calculate_final_transforms(frame1, frame2, frame3, frame4, frame5)
        self.precompute_blend_masks(frame1, frame2, frame3, frame4, frame5)
    ...
    def get_parameters(self) -> StitchingParameters5:
        ...
        return StitchingParameters5(
            ...
            transform_5_to_neutral=self.transform_5_to_neutral,
            ...
            blend_zone_45_start=self.blend_zone_45_start,
            blend_zone_45_end=self.blend_zone_45_end
        )

    def set_parameters(self, params: StitchingParameters5):
    ...
    def calibrate(self, calibration_file: str) -> None:
        ...
        first_stitched = self.stitch_frame_optimized(frame1, frame2, frame3, frame4, frame5)
    ...
    def process_with_params(self, frame1: np.ndarray, frame2: np.ndarray,
                            frame3: np.ndarray, frame4: np.ndarray,
                            frame5: np.ndarray) -> np.ndarray:
        """Обработка пяти кадров с использованием загруженных параметров"""
        stitched = self.stitch_frame_optimized(frame1, frame2, frame3, frame4, frame5)
